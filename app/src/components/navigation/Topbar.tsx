...
export function Topbar() {
  ...
  const notifications = notifStore.notifications.length > 0 ? notifStore.notifications : mockNotifications;
              ...
              <div className="absolute top-full left-0 right-0 mt-2 bg-popover border rounded-xl shadow-xl p-4 z-50">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {quickActions.map((action) => (
                    <button key={action.label} className="flex items-center gap-2 p-2 rounded-lg hover:bg-accent text-left transition-colors">
            ...
            <SheetContent>
              ...
              <div className="mt-4 space-y-3">
                {notifications.map((n: any) => (
                  <div key={n._id} className={`p-3 rounded-xl border transition-colors ${!n.isRead ? 'bg-primary/5 border-primary/20' : 'bg-card'}`}>
